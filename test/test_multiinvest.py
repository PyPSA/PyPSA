...
def test_max_growth_constraint(n):
    ...
    # test store and link grow limit
    n.mremove("Generator", n.generators.query('bus == "1"').index)
    n.remove("Carrier", gen_carrier)
    n.add("Bus", "1 battery")
    ...
    for year in n.investment_periods:
        ...
        n.add(
            "Link", "bus2 battery charger-{}".format(year),
            bus0=1,
            bus1="1 battery",
            p_nom_extendable=True,
            build_year=year,
        ...
        n.add(
            "Link",
            "My bus2 battery discharger-{}".format(year),
            bus0="1 battery",
            bus1=1,
            p_nom_extendable=True,
            build_year=year,
    ...
    discharger_i = n.links[n.links.index.str.contains("battery discharger")].index
    n.links.loc[discharger_i, "carrier"] = "battery discharger"
    n.links.carrier.fillna("battery charger", inplace=True)
    ...
    n.add("Carrier",
          ...
          max_growth=100)
    status, cond = n.lopf(pyomo=False, multi_investment_periods=True)
    assert all(n.stores.e_nom_opt.groupby(n.stores.build_year).sum()<=500)
    assert all(n.links.loc[discharger_i, "p_nom_opt"].groupby(n.links.build_year).sum()<=100)
