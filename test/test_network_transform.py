...
class TestSuffix:
    """Tests for the `suffix` argument of `Network.add`/`Network.remove` (#1650).

    Covers: scalar name + scalar suffix (single component); list name + scalar
    suffix (broadcast suffix); scalar name + list suffix (broadcast name); list
    name + list suffix (raises, was a silent one-by-one pairing). Also regresses
    the TypeError that fired when a list suffix met a Series/DataFrame kwarg.
    """

    def test_add_list_name_scalar_suffix(self, simple_network):
        n = simple_network
        n.add("Generator", ["a", "b"], suffix=" wind", bus="bus")
        assert list(n.c.generators.static.index) == ["a wind", "b wind"]

    def test_add_scalar_name_list_suffix(self, simple_network):
        n = simple_network
        n.add("Generator", "g", suffix=[" wind", " solar"], bus="bus")
        assert list(n.c.generators.static.index) == ["g wind", "g solar"]

    def test_add_list_name_list_suffix_raises(self, simple_network):
        n = simple_network
        with pytest.raises(ValueError, match="Cannot pass list to both"):
            n.add("Generator", ["a", "b"], suffix=[" wind", " solar"], bus="bus")

    def test_add_empty_list_suffix_raises(self, simple_network):
        n = simple_network
        with pytest.raises(ValueError, match="Empty list `suffix`"):
            n.add("Generator", "g", suffix=[], bus="bus")

    def test_add_scalar_name_list_suffix_with_series_kwarg(self, simple_network):
    ...
    def test_add_scalar_name_list_suffix_with_dataframe_kwarg(self, simple_network):
        ...
        n.add(
            "Generator", "g", suffix=[" wind", " solar"], bus="bus", p_max_pu=p_max_pu
        )
        assert list(n.c.generators.static.index) == ["g wind", "g solar"]
    ...
    def test_add_single_element_list_suffix_matches_scalar(self, simple_network):
        """A length-1 list suffix behaves identically to the scalar suffix: it
        yields a single component whose snapshot-indexed kwargs are time-varying."""
        n = simple_network
        p_max_pu = pd.Series([0.1, 0.2, 0.3], index=n.snapshots)
        n.add("Generator", "g", suffix=[" wind"], bus="bus", p_max_pu=p_max_pu)
        assert list(n.c.generators.static.index) == ["g wind"]
        pd.testing.assert_series_equal(
            n.c.generators.dynamic.p_max_pu["g wind"],
            p_max_pu,
            check_names=False,
    ...
    def test_remove_scalar_name_list_suffix(self, simple_network):
        n = simple_network
        n.add("Generator", ["g wind", "g solar"], bus="bus")
        n.remove("Generator", "g", suffix=[" wind", " solar"])
        assert list(n.c.generators.static.index) == []

    def test_remove_empty_list_suffix_raises(self, simple_network):
        n = simple_network
        with pytest.raises(ValueError, match="Empty list `suffix`"):
            n.remove("Generator", "g", suffix=[])

    def test_remove_list_name_list_suffix_raises(self, simple_network):
        ...
        with pytest.raises(ValueError, match="Cannot pass list to both"):
            n.remove("Generator", ["DE0 0", "DE0 1"], suffix=[" wind", " solar"])
