...
def test_time_dependent_generator_efficiency(api):
    ...
    n.add('Generator', 'gen', carrier='carrier', marginal_cost=1, bus='bus', p_nom=1, efficiency=s)
...
def test_time_dependent_standing_losses_storage_units(api):
    ...
    n.snapshots = range(len(s))
    n.add('Bus', 'bus')
    n.add('StorageUnit', 'su', bus='bus', marginal_cost=1, p_nom=1, max_hours=1, state_of_charge_initial=1, standing_loss=s)
    status, _ = optimize(n, api)
    assert status == "ok"
    equal(n.storage_units_t.state_of_charge.su.values, [1., 0.9, 0.72])
...
def test_time_dependent_standing_losses_stores(api):
    ...
    n.snapshots = range(len(s))
    n.add('Bus', 'bus')
    n.add('Store', 'sto', bus='bus', marginal_cost=1, e_nom=1, e_initial=1, standing_loss=s)
    status, _ = optimize(n, api)
    assert status == "ok"
    equal(n.stores_t.e.sto.values, [1., 0.9, 0.72])