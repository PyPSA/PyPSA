...
1. **Bug fix for issue #1220**: a fixed ``phase_shift_set`` is now included in
...
2. **New feature (issue #456)**: when ``phase_shift_min < phase_shift_max`` the
   ...
   dynamic output ``n.transformers_t["phase_shift"]``, while the input
   ``phase_shift_set`` is preserved.
...
def _build(
    ...
    n.add(
        ...
        r=1e-6,
        s_nom=100,
        phase_shift_set=phase_shift_set,
        **bounds,
    )
    return n
...
class TestFixedPhaseShiftInKVL:
    """Issue #1220 bug fix: fixed phase_shift_set respected by LOPF."""

    def test_baseline_equal_split(self):
        """No phase shift → 25/25 flow split (equal x_pu)."""
        n = _build(phase_shift_set=0.0)
    ...
    def test_negative_phase_shift_reverses_sign(self):
        ...
        n_pos = _build(phase_shift_set=10.0)
        n_pos.optimize(solver_name="highs")
        n_neg = _build(phase_shift_set=-10.0)
...
class TestVariablePhaseShift:
    ...
    def test_variable_preserves_set_input(self):
        """The input attribute `phase_shift_set` is not overwritten by the solve."""
        n = _build(phase_shift_set=3.0, variable=True)
        assert n.transformers.at["T1", "phase_shift_set"] == 3.0
        n.optimize(solver_name="highs")
        assert n.transformers.at["T1", "phase_shift_set"] == 3.0
...
class TestDeprecatedPhaseShiftInput:
    """`phase_shift` is a deprecated alias for the `phase_shift_set` input."""

    def test_add_phase_shift_aliases_to_set(self):
        n = pypsa.Network()
        n.set_snapshots([0, 1])
        n.add("Bus", "A")
        n.add("Bus", "B")
        with pytest.warns(DeprecationWarning, match="phase_shift_set"):
            n.add(
                "Transformer",
                "T1",
                bus0="A",
        ...
        assert n.transformers.at["T1", "phase_shift_set"] == 7.0
...
class TestLOPFMatchesLinearPowerFlow:
    ...
    @pytest.mark.parametrize("phase_shift_set", [-15.0, -5.0, 0.0, 5.0, 15.0])
    def test_lopf_flows_match_lpf(self, phase_shift_set):
        n = _build(phase_shift_set=phase_shift_set)
        n.optimize(solver_name="highs")
        lopf_L1 = n.lines_t.p0["L1"].copy()
        lopf_T1 = n.transformers_t.p0["T1"].copy()
        ref = _build(phase_shift_set=phase_shift_set)
        ref.lpf()
    ...
    def test_variable_result_consistent_with_lpf(self):
        ...
        lopf_L1 = n.lines_t.p0["L1"].copy()
        check = _build(phase_shift_set=opt_angle)
        check.lpf()
        np.testing.assert_allclose(
