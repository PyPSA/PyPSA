...
def test_add_network_static_not_inplace(ac_dc_network, empty_network_5_buses):
    ...
    THEN    we obtain a new network which should contain the buses of
...
def test_add_network_with_time_inplace():
    """
    GIVEN   two networks containing three buses each with time-varying
    attributes.

    WHEN    the second network is added to the first with with_time=True

    THEN    the first network should now contain the time-varying
    attributes of the         buses in the second network
    """
