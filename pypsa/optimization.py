...
def define_dispatch_variables(n, sns, c, attr):
    """
    ...
    n.model.add_variables(coords=[sns, n.df(c).index], name=f'{c}-{attr}', mask=active)


def define_nominal_variables_for_extendables(n, c, attr):
    """
    ...
    n.model.add_variables(lower, upper, name=f'{c}-{attr}')


def define_dispatch_constraints_for_non_extendables(n, sns, c, attr):
    """
    Sets power dispatch constraints for non-extendable devices for a given
...
def define_dispatch_constraints_for_extendables(n, sns, c, attr):
    """
...
#     active = get_activity_mask(n, c, sns)[com_i] if n._multi_invest else None
#     n.model.add_constraints(n, lhs, '<=', 0, 'Generators', 'committable_lb', mask=mask)

#     lhs = linexpr((upper, status), (-1, p))
#     n.model.add_constraints(n, lhs, '>=', 0, 'Generators', 'committable_ub')
...
#     n.model.add_constraints(n, lhs, sense, rhs, 'Bus', 'marginal_price')
...
# def prepare_lopf(n, snapshots=None, extra_functionality=None):
#     """
...
def network_lopf(n, snapshots=None, solver_name="cbc",
    ...
    prepare_lopf(n, snapshots, extra_functionality)
    # status, condition = n.model.solve(solver_name=solver_name,
    #                                   warmstart_fn=warmstart,
    #                                   log_fn=solver_logfile,
    #                                   keep_files=keep_files,
    #                                   **solver_options)

    # if status == "ok":
    #     assign_solution(n, snapshots)
    #     n.objective = n.model.objective
