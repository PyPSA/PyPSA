...
"""
Post-solving statistics of network.

This module contains functions to anaylize an optimized network. Basic
information of network can be summarized as well as constraint gaps can
be double-checked.
"""
...
def calculate_curtailment(n):
    ...
    avail = (
        max_pu.multiply(n.generators.p_nom_opt.loc[max_pu.columns])
        .sum()
        .groupby(n.generators.carrier)
        .sum()
    )
    used = n.generators_t.p[max_pu.columns].sum().groupby(n.generators.carrier).sum()
...
def describe_storage_unit_contraints(n):
    ...
    if "p_store" in pnl:
        ...
        logging.info(
            "Storage Unit SOC balance not reconstructable as no "
...
def check_constraints(n, tol=1e-3):
    ...
    constraints, it test whether the inequality is violated with a higher value
    then the tolerance.
    ...
    assert condition.empty, (
        f"The following constraint(s) are exceeding the "
