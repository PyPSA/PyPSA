...
class Components(
    ...
    def get_delay_source_indexer(
        ...
        >>> src, valid = Components.get_delay_source_indexer(snapshots, weightings, delay=2, is_cyclic=True)
        >>> src
        array([4, 5, 0, 1, 2, 3])
        >>> valid
        array([ True,  True,  True,  True,  True,  True])
        >>> src, valid = Components.get_delay_source_indexer(snapshots, weightings, delay=2, is_cyclic=False)
        >>> src
        array([0, 0, 0, 1, 2, 3])
        >>> valid
        ...
        if isinstance(snapshots, pd.MultiIndex):
            ...
            for period in snapshots.unique("period"):
                ...
                p_src, p_valid = Components._delay_positions(w, delay, is_cyclic)
        ...
        return Components._delay_positions(w, delay, is_cyclic)
