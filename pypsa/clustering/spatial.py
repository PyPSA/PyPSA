...
def aggregateoneport(
    ...
    non_aggregated = n.components[c].static[~to_aggregate]
    ...
    dynamic = {}
    if with_time:
        dynamic_strategies = align_strategies(strategies, n.components[c].dynamic, c)
        for attr, data in n.components[c].dynamic.items():
            if data.empty:
                dynamic[attr] = data
                continue
...
def aggregatelines(
    ...
    static = n.components["Line"].static
    ...
    if with_time:
        dynamic_strategies = align_strategies(
            strategies, n.components["Line"].dynamic, "Line"
        )
...
def get_clustering_from_busmap(
    ...
    for c in n.components[one_port_components]:
        if c.empty:
            continue
