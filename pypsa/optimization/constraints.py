...
def define_ramp_limit_constraints(n, sns, c, attr):
    ...
    if not limit_up.isnull().all():
        ...
        if is_rolling_horizon:
            ...
            lim_up = nominal*get_as_dense(n, c, "ramp_limit_up")[com_i].iloc[start_i].fillna(1)
            lim_start_up = nominal*get_as_dense(n, c, "ramp_limit_start_up")[com_i].iloc[start_i].fillna(1)
    ...
    if not limit_down.isnull().all():
        ...
        if is_rolling_horizon:
            status_start = n.pnl(c)["status"][com_i].iloc[start_i]
            lim_shut = nominal*get_as_dense(n, c, "ramp_limit_shut_down")[com_i].iloc[start_i].fillna(1)
