...
def define_nominal_constraints_for_extendables(
    ...
    """
    c = as_components(n, component)
    # Bounds of assets with a build decision are set in `define_build_constraints`
    ext_i = c.extendables.intersection(c.active_assets).difference(c._offset_assets)

    if ext_i.empty:
        return

    capacity = n.model[f"{c.name}-{attr}"].sel(name=ext_i)
...
def define_build_constraints(n: Network, component: str, attr: str) -> None:
    """Define capacity bounds scaled by the binary build decision.

    For each extendable asset with an offset cost, the capacity bounds only apply
    if the asset is built, and the capacity is zero otherwise:

    min_capacity * built <= capacity <= max_capacity * built

    These replace the bounds of `define_nominal_constraints_for_extendables`.
    Where the maximum capacity is infinite, the big-M fallback is used. For
    modular assets the modularity equality then forces the module count to zero.
    Committable, non-modular assets can additionally only be committed if built.
    ...
    built = m[f"{c.name}-built"]

    nom_min = c.da[attr + "_min"].sel(name=build_i)
    nom_max = c.da[attr + "_max"].sel(name=build_i)
    big_m = c._resolve_big_m_default(n._committable_big_m)
    cap_max = nom_max.where(np.isfinite(nom_max) & (nom_max > 0), big_m)

    capacity = m[f"{c.name}-{attr}"].sel(name=build_i)
    m.add_constraints(
        capacity >= nom_min * built, name=f"{c.name}-ext-{attr}-lower-built"
    )
    m.add_constraints(
        capacity <= cap_max * built, name=f"{c.name}-ext-{attr}-upper-built"
    )

    com_i = build_i.difference(c.modulars).intersection(c.committables)
    if com_i.empty:
        return

    status = m[f"{c.name}-status"].sel(name=com_i)
    m.add_constraints(status <= built.sel(name=com_i), name=f"{c.name}-status-built")
