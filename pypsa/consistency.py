...
def check_cost_consistency(component: Components, strict: bool = False) -> None:
    ...
    if {"capital_cost_offset", "overnight_cost_offset"}.issubset(static.columns):
        both_offset = static["overnight_cost_offset"].notna() & (
            static["capital_cost_offset"] != 0
        )
        if both_offset.any():
            assets = static.index[both_offset].tolist()
            _log_or_raise(
                strict,
                "Component %s has assets with both 'overnight_cost_offset' and "
                "'capital_cost_offset' set: %s. When 'overnight_cost_offset' is "
                "provided, it takes precedence and 'capital_cost_offset' is ignored. "
                "Consider setting capital_cost_offset=0 for these assets.",
                component.name,
...
def check_big_m_exceeded(n: Network, strict: bool = False) -> None:
    ...
    for c in n.components:
        ...
        if len(names):
            _log_or_raise(
                strict,
                "Optimized capacities reach the big-M fallback bound for %s with offset cost "
