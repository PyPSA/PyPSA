...
- Under linopy's v1 semantics, the multi-period `snapshot` dimension of `n.model` is a flat dimension with `period`/`timestep` auxiliary coordinates instead of a `pandas.MultiIndex`. Results are unchanged, and [`pypsa.options.optimization.model_snapshot_index`][pypsa.options] overrides the choice. The flat form becomes the default in `2.0`. `extra_functionality` receives the model's snapshot labels, so code reading them as a `MultiIndex` needs to change; see [custom constraints](./user-guide/optimization/custom-constraints.md#handling-snapshots-in-multi-investment-optimizations) for the migration. (<!-- md:pr 1829 -->)
