...
- Add offset costs for extendable `Generator`, `Link`, `Line`, `Process`, `Store` and `StorageUnit` components via the new attributes `capital_cost_offset` and `overnight_cost_offset`. These investment costs are paid once if an asset is built, independent of its capacity (e.g. grid connection or land acquisition). A non-zero offset cost introduces a binary build decision per asset, which also makes `p_nom_min` apply only if the asset is built. The optimised decision is written to the new static output `built`. See the [capacity limits user guide](./user-guide/optimization/capacity-limits.md#build-decisions) for the formulation. (<!-- md:pr 1900 -->)
