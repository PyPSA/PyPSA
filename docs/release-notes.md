...
- Add opt-in capacity degradation and cycle budgets for [StorageUnit](./user-guide/components/storage-units.md) and [Store](./user-guide/components/stores.md) components through three new attributes. With `degradation_per_cycle > 0` (capacity fade per equivalent full cycle) or a finite `cycles_max` the cumulative energy throughput is tracked as a state variable, written to `n.storage_units_t.throughput` / `n.stores_t.throughput`; `throughput_initial` carries the throughput before the horizon. The fade derates the ceiling of the storage level by the degradation accrued so far. `cycles_max` caps the cumulative throughput, `throughput_initial` included, so the budget spans rolling-horizon windows, with its shadow price in `mu_cycles_max`. Both stay linear for extendable assets. Stores count `|p|` through a hidden auxiliary `p_dispatch` built only for stores using these features. The helpers `set_cycle_life(cycles_life, soh_end)` (cycle life to attributes) and `get_cycles()` (cycles performed) are available on both components. Off by default. See the [:material-book-open-variant: user guide](./user-guide/optimization/storage.md#capacity-degradation-and-cycle-budgets). ([#1731](https://github.com/PyPSA/PyPSA/issues/1731))
