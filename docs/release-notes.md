...
- Clarify the `Store` `marginal_cost` and `marginal_cost_quadratic` documentation. The cost applies to the net dispatch variable `p` (positive when discharging incurs cost, negative when charging is credited), not symmetrically to both charging and discharging as the previous wording implied. (<!-- md:pr 1774 -->)
