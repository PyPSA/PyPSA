...
- [Store](./user-guide/components/stores.md) components now cover all functionality of [StorageUnit](./user-guide/components/storage-units.md) components. Stores gain a charging variable `p_store` next to the dispatch variable `p` with the efficiencies `efficiency_dispatch` and `efficiency_store`; a finite `max_hours` couples the power limits to the energy capacity `e_nom`; and `inflow` with `spill_cost` model exogenous inflow and spillage. The new attributes `marginal_cost_dispatch` and `marginal_cost_store` price discharging and charging separately, while `marginal_cost` keeps applying to the net dispatch. The outputs `p_dispatch`, `p_store` and `spill` are added. Existing store models, their results and the model variable `Store-p` (net dispatch) are unchanged. The `StorageUnit` component is deprecated and will be removed in PyPSA 2.0; adding storage units raises a `FutureWarning`. Storage units can be converted into equivalent stores with [`n.storage_units_to_stores()`][pypsa.Network.storage_units_to_stores]. See [#1728](https://github.com/PyPSA/PyPSA/issues/1728).
