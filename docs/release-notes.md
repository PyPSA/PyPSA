...
- Add [`n.statistics.emissions()`][pypsa.statistics.StatisticsAccessor.emissions], which accounts emissions of generators, storage units and stores from the carrier attribute `co2_emissions` in the same way as the `primary_energy` [global constraint](./user-guide/optimization/global-constraints.md), so that its sum reproduces the emissions the constraint limits. (<!-- md:pr 1913 -->)
