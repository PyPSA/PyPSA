...
- Fix misleading warnings about cyclic storage overriding initial levels. When `cyclic_state_of_charge=True` (`e_cyclic=True`) was combined with `state_of_charge_initial_per_period=True` (`e_initial_per_period=True`), PyPSA warned that the user-defined initial level would be ignored, while the model in fact seeds the level at the start of every investment period. The warning is now only raised when the initial level really is discarded, and a new warning states that per-period seeding overrides global cycling. The flag precedence `cyclic_state_of_charge_per_period` > `state_of_charge_initial_per_period` > `cyclic_state_of_charge` (analogously for `Store`) is now documented in the component attributes; the optimisation results are unchanged.
