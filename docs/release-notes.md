...
- Add [`SubNetwork.calculate_zonal_PTDF`][pypsa.SubNetwork.calculate_zonal_PTDF] to derive a zonal PTDF (`branch x zone`) from the nodal PTDF and a generation shift key, with the builders [`gsk_uniform`][pypsa.SubNetwork.gsk_uniform] and [`gsk_by_capacity`][pypsa.SubNetwork.gsk_by_capacity]. See the [:material-book-open-variant: user guide](./user-guide/optimization/power-flow.md#zonal-ptdf-and-generation-shift-keys).
