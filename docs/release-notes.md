...
- Add a `FlowBasedDomain` component for flow-based market coupling: it bounds the net positions of market zones (buses) by linear inequalities `zonal_ptdf . NP <= RAM`, one row per critical network element (CNEC). See the [:material-book-open-variant: user guide](./user-guide/components/flow-based-domains.md).
    - The zonal PTDF matrix (CNEC x zone) is passed to [`n.add`][pypsa.Network.add] as `zonal_ptdf=<DataFrame>` and read back from `n.c.flow_based_domains.zonal_ptdf`; the `ram` right-hand side may be static or time-varying.
