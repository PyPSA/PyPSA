...
- Add capacity degradation and cycle budgets for [StorageUnit](./user-guide/components/storage-units.md) and [Store](./user-guide/components/stores.md) components via the new attributes `degradation_per_cycle`, `cycles_max` and `throughput_initial`. The cumulative energy throughput is tracked as a new variable `throughput`, which derates the ceiling of the storage level and is capped by `cycles_max`. The helpers `set_cycle_life()` and `get_cycles()` translate a cycle life into these attributes and return the cycles performed. See the [:material-book-open-variant: user guide](./user-guide/optimization/storage.md#capacity-degradation-and-cycle-budgets). (<!-- md:pr 1915 -->)
