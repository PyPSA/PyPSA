...
- Add [`n.statistics.lcoe()`][pypsa.statistics.StatisticsAccessor.lcoe], the levelised cost of the energy a component supplies. The cost includes the component's capital, fixed and operational expenditures plus the energy it buys at its other ports, and credits the by-products it sells there, all valued at marginal prices. Pass `bus_carrier` to levelise over one output, e.g. `bus_carrier="hydrogen"` for the levelised cost of hydrogen from an electrolyser, or `bus_carrier="heat"` for a CHP, crediting its electricity sales. The gap to the price the component captures is its profit per unit of energy. (<!-- md:pr 1926 -->)
- Add [`n.statistics.profit()`][pypsa.statistics.StatisticsAccessor.profit], the revenue of a component minus its capital, fixed and operational expenditures. (<!-- md:pr 1926 -->)
