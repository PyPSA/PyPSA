...
- Fix flow-direction arrows on the interactive Pydeck map ([`n.explore`][pypsa.Network.explore]) being distorted by a factor of `sec(latitude)` (roughly 2x at 60°N). The arrows were projected to Web Mercator (EPSG:3857) but inverted with a spherical approximation; they are now inverted with the same projection, so their size and position are consistent with the basemap and independent of latitude. (<!-- md:pr 1783 -->)
