...
- Add `include_objective_constant` parameter to `n.optimize()` and `n.optimize.create_model()` to control whether the objective constant (capital costs of existing infrastructure) is included as a variable in the objective function. Setting to `False` improves LP numerical conditioning. The default will change from `True` to `False` in a future version. (<!-- md:pr 503 -->)
