...
- Under linopy's v1 semantics, the multi-period `snapshot` dimension of `n.model` is a flat dimension with `period`/`timestep` auxiliary coordinates instead of a `pandas.MultiIndex`. Results are unchanged, and [`pypsa.options.optimization.model_snapshot_index`][pypsa.options] overrides the choice. The flat form becomes the default in `2.0`. See [custom constraints](./user-guide/optimization/custom-constraints.md#snapshots-in-the-model) for migrating `extra_functionality` code. (<!-- md:pr 1829 -->)
### Bug Fixes
