...
- Add a `FlowBasedConstraint` component for flow-based market coupling: it bounds the net positions of market zones (buses) by linear inequalities `zonal_ptdf . NP <= RAM`, one row per critical network element (CNEC). See the [:material-book-open-variant: user guide](./user-guide/components/flow-based-constraints.md).
    - The zonal PTDF matrix (CNEC x zone) is passed to [`n.add`][pypsa.Network.add] as `zonal_ptdf=<DataFrame>` and read back from `n.c.flow_based_constraints.zonal_ptdf`; the `ram` right-hand side may be static or time-varying. The zonal PTDF may itself be time-varying: pass a `(snapshot, CNEC)` MultiIndex frame and the constraint broadcasts over snapshots.
    - A zonal PTDF can be derived from a nodal, grid-resolved network with [`SubNetwork.calculate_zonal_PTDF`][pypsa.SubNetwork.calculate_zonal_PTDF] (`= nodal PTDF · GSK`), returning a labelled `branch x zone` frame. The generation shift key is a `bus x zone` frame or a scheme name resolved by the `gsk_uniform` / `gsk_by_capacity` builders.
    - Each zone's net position is a variable inside the nodal balance, so zonal prices stay native (no auxiliary buses or links); it is written to `n.buses_t.net_position` and, absent corridors, equals `n.buses_t.p`.
    - A domain column may instead name a `Link`, whose `Link-p` flow enters the constraint directly, covering advanced hybrid coupling (a link to an external hub) and evolved flow-based coupling (a link between two zones). The corridor's Core-side term is cancelled in the nodal balance so it loads the CNECs only through its own column (net positions stay `generation - load`); an AHC border also joins the zero-sum balance as its external hub's net position. AHC vs EvFB is inferred from the link endpoints.
    - Per-CNEC shadow prices are written to the `mu_domain` output when optimizing with `assign_all_duals=True`.
    - Published domains can be imported with `n.c.flow_based_constraints.from_eraa(path, year, season)` (ERAA `FB-Domain-CORE` workbooks; needs the `excel` extra), `.from_jao(path)` (JAO `finalComputation` CSV), and `.from_tso(path)` (TSO `MS_FBMC` domain CSV, self-typed via its `OBJEKTTYP` row, Latin-1 with auto-detected decimal separator). All accept a `links` mapping to include AHC/EvFB corridors (ERAA aligns the sign to the link orientation automatically). For ERAA, `season` may be a `pandas.Series` mapping each snapshot to a season name, building a time-varying domain from the union of the seasons' CNECs.
