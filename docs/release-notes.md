...
- Added [`n.da`][pypsa.Network.da], an xarray accessor for the network's own time series, mirroring the component accessor `n.c.<component>.da`. It currently exposes the snapshot weightings as `n.da.snapshot_weightings.<kind>`. While an optimization model is live the arrays carry the model's snapshot labels, so custom constraints can weight model objects without knowing the snapshot representation.

- Prepared the optimization module for linopy's upcoming v1 arithmetic convention (strict coordinate alignment and NaN handling). Expression combinations that previously relied on linopy's implicit alignment now align operands explicitly (via `join=`, `.sel`, and `.fillna` on constants), so model building runs correctly under both the current and the new linopy semantics. This is a no-op for results and remains compatible with released linopy `0.9.x`.

- The multi-period `snapshot` representation in `n.model` now follows linopy's active semantics. Under the current/legacy semantics (and any linopy without the `semantics` option) the `snapshot` dimension stays a `pandas.MultiIndex` as before. Only when linopy's v1 convention is enabled (`linopy.options["semantics"] = "v1"`, the option is introduced in linopy `0.10`) — which forbids a `MultiIndex` dimension coordinate — is it built as a flat dimension with `period`/`timestep` auxiliary coordinates. Results (`n.*_t`) are unchanged either way. Custom `extra_functionality` selecting on the `snapshot` `MultiIndex` keeps working under legacy but must migrate to the auxiliary coordinates under v1; the flat representation becomes PyPSA's default in `2.0`. The representation can also be set explicitly via [`pypsa.options.optimization.snapshot_representation`][pypsa.options] (`"auto"` default, `"flat"`, `"multiindex"`). It is fixed when the model is created and stays stable for the model's lifetime, so code extending `n.model` outside of `n.optimize()` sees the same representation the model was built with.

    Representation-agnostic code is written against the `da` accessors, which hand out data on the labels of the live model: `n.c.<component>.da.<attr>` for component attributes and the new [`n.da.snapshot_weightings.<kind>`][pypsa.Network.da] for the snapshot weightings. Grouping by investment period works as `expr.groupby("period").sum()` under either representation. For data that has to stay in pandas, [`n.optimize.window`][pypsa.optimization.window.SnapshotWindow] holds the build's snapshots in both labellings and bridges them: `window.is_flat` reports the representation, `window.on_model(df)` puts a snapshot-indexed object on the model's labels, `window.network_index` and `window.model_index` hold the build's snapshots in either labelling, and `window.iter_periods()` yields the per-investment-period snapshots. All of it degenerates to identity outside a flat multi-period build. See [custom constraints](./user-guide/optimization/custom-constraints.md#snapshots-in-the-model) for a migration example.

- `pypsa.statistics.port_efficiency` gained an `as_xarray` argument. With `as_xarray=True` the efficiency is returned as an `xarray.DataArray` on the labels of the live optimization model, so port coefficients align with the model under both linopy semantics. The default (`as_xarray=False`) is unchanged and still returns a `pandas.Series` or `pandas.DataFrame` indexed by `n.snapshots`.
