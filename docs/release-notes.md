...
- The multi-period `snapshot` dimension of `n.model` now follows linopy's active semantics: a `pandas.MultiIndex` as before, or a flat dimension with `period`/`timestep` auxiliary coordinates once linopy's v1 convention is enabled (`linopy.options["semantics"] = "v1"`, introduced in linopy `0.10`), which forbids a `MultiIndex` dimension coordinate. Results (`n.*_t`) are unchanged either way, and [`pypsa.options.optimization.snapshot_representation`][pypsa.options] selects the representation explicitly. The flat representation becomes the default in `2.0`; see [custom constraints](./user-guide/optimization/custom-constraints.md#snapshots-in-the-model) for migrating `extra_functionality` that selects on the `snapshot` `MultiIndex`. (<!-- md:pr 1829 -->)
