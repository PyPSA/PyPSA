...
- The multi-period `snapshot` representation in `n.model` now follows linopy's active semantics. Under the current/legacy semantics (and any linopy without the `semantics` option) the `snapshot` dimension stays a `pandas.MultiIndex` as before. Only when linopy's v1 convention is enabled (`linopy.options["semantics"] = "v1"`, the default from linopy `1.0`) — which forbids a `MultiIndex` dimension coordinate — is it built as a flat dimension with `period`/`timestep` auxiliary coordinates. Results (`n.*_t`) are unchanged either way. Custom `extra_functionality` selecting on the `snapshot` `MultiIndex` keeps working under legacy but must migrate to the auxiliary coordinates under v1; the flat representation becomes PyPSA's default in `2.0`.
