...
- `pypsa.statistics.port_efficiency(..., dynamic=True)` now returns an `xarray.DataArray` on the labels of the live optimization model instead of a `pandas.DataFrame` indexed by `n.snapshots`, so port coefficients align with the model under both linopy semantics. The static form (`dynamic=False`, the default) is unchanged and still returns a `pandas.Series`.
