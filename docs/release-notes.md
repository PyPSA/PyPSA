...
- The multi-period `snapshot` representation in `n.model` now follows linopy's active semantics. Under the current/legacy semantics (and any linopy without the `semantics` option) the `snapshot` dimension stays a `pandas.MultiIndex` as before. Only when linopy's v1 convention is enabled (`linopy.options["semantics"] = "v1"`, the option is introduced in linopy `0.10`) — which forbids a `MultiIndex` dimension coordinate — is it built as a flat dimension with `period`/`timestep` auxiliary coordinates. Results (`n.*_t`) are unchanged either way. Custom `extra_functionality` selecting on the `snapshot` `MultiIndex` keeps working under legacy but must migrate to the auxiliary coordinates under v1; the flat representation becomes PyPSA's default in `2.0`. The representation can also be set explicitly via [`pypsa.options.optimization.snapshot_representation`][pypsa.options] (`"auto"` default, `"flat"`, `"multiindex"`). It is fixed when the model is created and stays stable for the model's lifetime, so code extending `n.model` outside of `n.optimize()` sees the same representation the model was built with.
    ...
    The public entry point for writing representation-agnostic code is the new [`n.optimize.window`][pypsa.optimization.window.SnapshotWindow], which holds the snapshots of the live model build in both the network's and the model's labelling: `window.is_flat` reports the representation, `window.on_model(df)` puts a snapshot-indexed pandas object on the model's labels (`window.on_network(df)` restricts one to the build's snapshots, keeping the network's labels), `window.model_weightings("objective")` does the same for the snapshot weightings, and `window.iter_periods()` yields the per-investment-period snapshots. All of it degenerates to identity outside a flat multi-period build. See [custom constraints](./user-guide/optimization/custom-constraints.md#snapshots-in-the-model) for a migration example.
