...
- Add [`n.statistics.congestion_rent()`][pypsa.statistics.StatisticsAccessor.congestion_rent], which reports the congestion rent (merchandising surplus) collected on [Line](./user-guide/components/lines.md), [Link](./user-guide/components/links.md), [Process](./user-guide/components/processes.md) and [Transformer](./user-guide/components/transformers.md) components, i.e. the value of the power a branch delivers minus the value of the power it withdraws, evaluated at the marginal prices of the connected buses. (<!-- md:pr 1924 -->)
