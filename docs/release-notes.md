...
- The multi-period `snapshot` dimension of `n.model` now follows linopy's active semantics: a `pandas.MultiIndex` as before, or a flat dimension with `period`/`timestep` auxiliary coordinates once linopy's v1 convention is enabled (`linopy.options["semantics"] = "v1"`, introduced in linopy `0.10`), which forbids a `MultiIndex` dimension coordinate. Results (`n.*_t`) are unchanged either way, and [`pypsa.options.optimization.snapshot_representation`][pypsa.options] selects the representation explicitly. The flat representation becomes the default in `2.0`. `extra_functionality` keeps receiving the network's snapshots, so slicing them and selecting the model with the result works under both; only code reading the model's own snapshot index as a `MultiIndex` needs to change, see [custom constraints](./user-guide/optimization/custom-constraints.md#handling-snapshots-in-multi-investment-optimizations). (<!-- md:pr 1829 -->)
- Add [`n.da`][pypsa.Network.da], an xarray accessor for the network's own time series mirroring `n.c.<component>.da`. It exposes the snapshot weightings as `n.da.snapshot_weightings.<kind>` on the labels of the live optimization model, so custom constraints need not know the snapshot representation. The same holds for [`n.optimize.window`][pypsa.optimization.window.SnapshotWindow], which carries the build's snapshots in both labellings for data that stays in pandas. (<!-- md:pr 1829 -->)
- `pypsa.statistics.port_efficiency` gained an `as_xarray` argument. With `as_xarray=True` the efficiency is returned on the labels of the live optimization model instead of a pandas object indexed by `n.snapshots`. (<!-- md:pr 1829 -->)
