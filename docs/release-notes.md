...
- Default values for `ramp_limit_start_up` and `ramp_limit_shut_down` changed from `1` to `NaN` for generators and links. This allows distinguishing between "no limit" (NaN) and "full ramp allowed" (1). Code that relied on the implicit default of 1 may need updating. (<!-- md:pr 1553 -->)
- Ramp limit constraint names simplified from `{c}-fix-p-ramp_limit_*`, `{c}-ext-p-ramp_limit_*`, `{c}-com-p-ramp_limit_*` to unified `{c}-p-ramp_limit_*` (where `{c}` is `Generator` or `Link`). (<!-- md:pr 1553 -->)
