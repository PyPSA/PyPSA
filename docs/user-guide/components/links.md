...
The [`Link`](/api/components/types/links) components are used for controllable directed flows between two or
...
- The [`Link`](/api/components/types/links) component has one dispatch variable `p0` associated with the input
...
!!! note "When to use [`Line`](/api/components/types/lines) instead?"

    Use the [`Line`](/api/components/types/lines) component type for power lines for which their power flow is determined passively through Kirchhoff's voltage law.

!!! example "[`Link`](/api/components/types/links) with bidirectional lossless flow"

    Because the [`Link`](/api/components/types/links) component can have efficiency losses and marginal costs, the default settings allow only for flow in one direction (`p_min_pu=0`), from `bus0` to `bus1`. To build a bidirectional lossless link, set `efficiency = 1`, `marginal_cost = 0` and `p_min_pu = -1`.

!!! example "[`Link`](/api/components/types/links) with a single input and multiple outputs"
