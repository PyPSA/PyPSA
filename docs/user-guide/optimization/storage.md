...
| `cycles_max` | cycles | inf | cycle budget: equivalent full cycles allowed in total, `throughput_initial` included |

Either attribute alone tracks the throughput: a positive `degradation_per_cycle` with `cycles_max = inf` gives the capacity fade alone, a finite `cycles_max` with `degradation_per_cycle = 0` gives the cycle budget alone, and since `set_cycle_life` sets both, reset `cycles_max` to `inf` after calling it to keep only the fade.
...
**Throughput.** For every asset with a positive `degradation_per_cycle` or a finite `cycles_max` the cumulative throughput is tracked as a decision variable, starting from `throughput_initial` $Q^0_{n,s}$. Throughput is age: it is never cyclic and is not reset per investment period, whatever the storage level does; with rolling-horizon optimisation the last value of one window becomes `throughput_initial` of the next. It is written to `n.storage_units_t.throughput` and `n.stores_t.throughput`.

**Capacity fade.** With `degradation_per_cycle` $k_{n,s} > 0$ the ceiling of the storage level is derated by the fade accrued so far: the state of health falls linearly with cycles, so the capacity available at snapshot $t$ is $E \left(1 - k_{n,s} \tfrac{Q_{n,s,t}}{2E}\right) = E - \tfrac{k_{n,s}}{2} Q_{n,s,t}$, and the nominal capacity cancels out of the fade term.

**Cycle budget.** With a finite `cycles_max` $N^{\max}_{n,s}$ the throughput at the last active snapshot $T$, and with it at every snapshot, may not exceed $2 E N^{\max}_{n,s}$. The budget is cumulative: `throughput_initial` counts against it, so it covers the whole horizon with investment periods and all windows together with rolling-horizon optimisation. Its shadow price is written to `n.storage_units.mu_cycles_max` and `n.stores.mu_cycles_max`.
...
| $Q_{n,s,T} \leq 2 r_{n,s} H_{n,s} N^{\max}_{n,s}$ | `n.storage_units.mu_cycles_max` | `StorageUnit-cycles_max` |
...
| $Q_{n,s,T} \leq 2 E_{n,s} N^{\max}_{n,s}$ | `n.stores.mu_cycles_max` | `Store-cycles_max` |

**From technology data.** For a cycle life $N$ at an end-of-life state of health $\sigma$ (for instance 6000 cycles to 80 %) and a technical `lifetime` $L$ in years, `n.c.storage_units.set_cycle_life(N, soh_end=σ)` (likewise on `n.c.stores`) sets `degradation_per_cycle` $= (1 - \sigma) / N$ and `cycles_max` $= c + N \cdot \left(\sum_t w_t^s / 8760\right) / L$, the share of the cycle life that the snapshots may use on top of the $c$ cycles already performed, where an asset that has performed $c$ full cycles on capacity $E$ has `throughput_initial` $= 2 E c$; assets with an infinite `lifetime` keep `cycles_max = inf`. After the solve, `get_cycles()` on either component returns the equivalent full cycles performed over the snapshots from the solved dispatch, with or without these constraints.
