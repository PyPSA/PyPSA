...
A non-zero [offset cost](objective.md#offset-costs) (`capital_cost_offset` or `overnight_cost_offset`) on an extendable asset introduces a binary build decision $y \in \mathbb{B}$, which decides *whether* the asset is built,
separately from the continuous or integer decision on *how much* capacity is built.

The build decisions are available as `n.model.variables['{Component}-built']` and are written back to the static output `built` (e.g. `n.generators.built`),
which is `NaN` for assets without offset cost.
The build decision is a single decision per asset.
It carries neither a snapshot nor a scenario dimension, so an asset is either built for the whole horizon or not at all.

Like `capital_cost`, the offset cost only applies to extendable assets and is ignored for assets with a fixed capacity.

### Capacity Bounds

For assets with a build decision, the capacity bounds of [Upper and Lower Bounds](#upper-and-lower-bounds) are replaced by bounds scaled with the build decision.
The capacity is zero unless the asset is built:

| Constraint | Dual Variable | Name |
|-------------------|------------------|------------------|
| $G_{n,s} \leq \bar{G}_{n,s} \cdot y_{n,s}$ | N/A | `Generator-ext-p_nom-upper-built` |
| $F_{l} \leq \bar{F}_{l} \cdot y_{l}$ | N/A | `Link-ext-p_nom-upper-built` |
| $P_{l} \leq \bar{P}_{l} \cdot y_{l}$ | N/A | `Line-ext-s_nom-upper-built` |
| $E_{n,s} \leq \bar{E}_{n,s} \cdot y_{n,s}$ | N/A | `Store-ext-e_nom-upper-built` |
| $H_{n,s} \leq \bar{H}_{n,s} \cdot y_{n,s}$ | N/A | `StorageUnit-ext-p_nom-upper-built` |

Where the maximum capacity is infinite, the [big-M constant](#big-m-parameter-configuration) is used in its place.

The lower capacity bound only applies if the asset is built:

| Constraint | Dual Variable | Name |
|-------------------|------------------|------------------|
| $G_{n,s} \geq \underline{G}_{n,s} \cdot y_{n,s}$ | N/A | `Generator-ext-p_nom-lower-built` |
| $F_{l} \geq \underline{F}_{l} \cdot y_{l}$ | N/A | `Link-ext-p_nom-lower-built` |
| $P_{l} \geq \underline{P}_{l} \cdot y_{l}$ | N/A | `Line-ext-s_nom-lower-built` |
| $E_{n,s} \geq \underline{E}_{n,s} \cdot y_{n,s}$ | N/A | `Store-ext-e_nom-lower-built` |
| $H_{n,s} \geq \underline{H}_{n,s} \cdot y_{n,s}$ | N/A | `StorageUnit-ext-p_nom-lower-built` |

!!! tip "Minimum capacity without offset cost"

    To require "nothing or at least `p_nom_min`" without an actual offset cost, set a negligible offset cost (e.g. `capital_cost_offset=1e-3`).

For [modular](#modularity-constraints) assets, the modularity constraint $G_{n,s} = G^{\textrm{mod}}_{n,s} \cdot \tilde{G}_{n,s}$ then forces the module count to zero whenever the asset is not built.
The offset cost is paid once per asset, not per module.

### Build Decisions and Unit Commitment

Committable assets with an offset cost use the formulation of [Committable and Extendable Components](#committable-and-extendable-components) unchanged, including `linearized_unit_commitment=True`, which relaxes the status but not the build decision.
The capacity bound already forces zero dispatch when the asset is not built.
For non-modular assets, one constraint additionally prevents an asset from being committed if it is not built:

| Constraint | Dual Variable | Name |
|-------------------|------------------|------------------|
| $u_{n,s,t} \leq y_{n,s}$ | N/A | `Generator-status-built` |
| $u_{l,t} \leq y_{l}$ | N/A | `Link-status-built` |

!!! note "Initial status of committable assets"

    By default, a committable asset is assumed to be committed before the optimisation horizon starts, which can implicitly force it to be built.
    This happens for [modular committable](#modular-and-committable-components) assets (`status` defaults to 1) and for non-modular ones with `min_up_time > 0` (`up_time_before` defaults to 1).
    Set `status=0` and `up_time_before=0` to leave the build decision entirely to the optimiser.

These constraints are set in `define_build_constraints()`.

??? note "Mapping of symbols to component attributes"

    | Symbol | Attribute | Type |
    |-------------------|-----------|-------------|
    | $y_{n,s}$         | `n.generators.built` | Decision variable (binary) |
    | $y_{l}$           | `n.{links,lines}.built` | Decision variable (binary) |
