...
+ \sum_{l} L_{n,l,t} f_{l,t} + \sum_{m} R_{n,m,t} r_{m,t} + \sum_{l} K_{n,l} p_{l,t} = \sum_{s} d_{n,s,t} \quad \leftrightarrow  \quad w_t^o\lambda_{n,t}\end{gather*}$$
...
- $r_{m,t}$ for the internal dispatch of [`Process`][pypsa.components.Processes] components
...
The **incidence matrices** $K_{n,l}$ for the [`Line`][pypsa.components.Lines] and [`Transformer`][pypsa.components.Transformers] components, $L_{n,l,t}$ for the [`Link`][pypsa.components.Links] components and $R_{n,m,t}$ for the [`Process`][pypsa.components.Processes] components govern flows between buses. The incidence matrix $K_{n,l}$ takes non-zero values $-1$ if the line or transformer $l$ starts at bus $n$ and $1$ if it ends at
...
For processes, the time-varying incidence matrix $R_{n,m,t}$ is built from `rate{i}` attributes. At each connected `bus{i}`, the contribution is $-\text{rate}_{m,i,t}\, r_{m,t}$ (and analogously for delayed ports).
...
??? note "Mapping of symbols to component attributes"
    ...
    | $R_{n,m,t}$ | Calculated internally from `rate{i}` attributes | Parameter |
    | $\delta$ | `delay`, `delay2`, ... on `n.links` and `n.processes` | Parameter |
