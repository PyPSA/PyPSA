...
gen_b_cost = pd.Series(
    [1.0001, 1.00, 1.00, 1.00, 1.00, 1.00, 1.0001], index=snap
)  # epsilon tie-break
...
# --- Generators ---------------------------------------------------------------
n.add("Generator", "Gen_A", bus="A", p_nom=100, marginal_cost=gen_a_cost, carrier="gas")
n.add("Generator", "Gen_B", bus="B", p_nom=100, marginal_cost=gen_b_cost, carrier="gas")
...
axs[1].plot(
    x, trafo_b_cost.values, marker=".", linestyle=":", label="Transformer B cost"
)
...
patch_handles = [
    Patch(color=color, alpha=0.5, label=label)
