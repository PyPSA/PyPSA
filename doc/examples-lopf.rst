...
This section contains examples of network optimization with PyPSA. Note that PyPSA allows you to optimize networks in three ways: based on Linopy, based on Pyomo, and based on custom code. PyPSA's default optimization framework is based on Linopy. In earlier versions, PyPSA used the ``pyomo`` interface and an in-house implementation, both available through the `lopf` function. The latter are still available, but will be deprecated in the future. The following examples all use the Linopy-based implementation.
