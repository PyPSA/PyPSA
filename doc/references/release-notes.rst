...
* Two new attributes for the component `Generator` were added: `e_sum_min` and `e_sum_max`. These attributes allow to set the minimum and maximum total energy that can be generated by the generator over one optimization horizon. 
