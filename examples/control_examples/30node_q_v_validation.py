...
L = pd.Series([-0.00001]*1  + [-0.00002]*1 + [-0.00003]*1 + [-0.00005]*1 +[-0.00007]*2+ [-0.00008]*1 +[-0.0001]*2+ [-0.00021]*2 + [-0.00022]*2 + [-0.00023]*2  , snapshots)
L1 = pd.Series([0.007]*1  + [0.0066]*1 + [0.006]*1 + [0.0055]*1 +[0.005]*2+ [0.0045]*1 +[0.00499]*1+ [0.0047]*1+ [0.0043]*1 +[0.002]*1 + [0.0008]*1 +[0.0003]*1 +[0.00009]*1 + [0.00001]*1  , snapshots)
...
for i in range(n_buses):
    network.add("Load", "My load {}".format(i), bus="My bus {}".format((i+1) % n_buses), sn=0.00025,
...
qcontrol = Results_q.loc[:, "q_set_controller_output 0" : "q_set_controller_output 28"]

plt.plot(vmag, qdroop, label="Q(U) droop characteristic", color='r')
plt.scatter(v_control, qcontrol, color="g", label="Q(U) Controller characteristic")

# plt.xticks(v_control, color='g')
# plt.yticks(qcontrol, labels=qcontrol, color='g')

plt.legend(loc="best", bbox_to_anchor=(0.4, 0.4))
plt.title("Q(U) Controller Behavior \n  30 node example, \n snapshots = 15\n while loop condition = 1e-4 \n number of required iterations 76")
