...
L = pd.Series([-0.0000038]*1  + [-0.0000037]*1 + [-0.0000036]*1 + [-0.0000035]*1 +[-0.0000033]*1 +[-0.0000033]*1+ [-0.0000032]*1 +[-0.0000031]*1+[-0.0000031]*1+ [-0.0000030]*1 +[-0.0000030]*1+ [-0.0000030]*1 +[-0.0000030]*1+ [-0.0000030]*1+[-0.0000030]*1  , snapshots)
L1 = pd.Series([0.000013 ]*1  + [0.0000120]*1 + [0.0000115]*1 + [0.0000100]*1 +[0.0000090]*1 +[0.0000090]*1+ [0.0000089]*1 +[0.0000080]*1+ [0.0000060]*1+ [0.0000040]*1 +[0.0000035]*1 + [0.0000035]*1 +[0.0000035]*1 +[0.0000035]*1 + [0.0000035]*1  , snapshots)
...
for i in range(n_buses):
    network.add("Load", "My load {}".format(i), bus="My bus {}".format((i+1) % n_buses),
                p_set=L1, v1=0.89, v2=0.94, v3=0.96, v4=1.02)
...
for i in range(n_buses):
    network.add("Generator", "My Gen {}".format(i), bus="My bus {}".format((i+1) % n_buses), control="PQ", p_set=L, 
                sn=0.000004, v1=0.89, v2=0.94, v3=0.96, v4=1.02, damper=1)

def power_flow():
    network.lpf(network.snapshots)
    network.pf(use_seed=True, snapshots=network.snapshots, x_tol_outer=1e-3)
