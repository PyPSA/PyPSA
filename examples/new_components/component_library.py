...
override_components.loc["ShadowPrice"] = [
    "shadow_prices",
    "Shadow price for a global constraint.",
    np.nan,
]
...
override_component_attrs["ShadowPrice"] = pd.DataFrame(
    columns=["type", "unit", "default", "description", "status"]
)
override_component_attrs["ShadowPrice"].loc["name"] = [
    "string",
    "n/a",
    "n/a",
    "Unique name",
...
override_component_attrs["ShadowPrice"].loc["value"] = [
    "float",
    "n/a",
    0.0,
    "shadow value",
        ...
        def extra_func(network, snapshots):
            ...
            if not network.chps.empty:
                ...
                def chp_nom(model, chp):
                    return (
                        network.chps.at[chp, "eta_elec"]
                        * network.chps.at[chp, "p_nom_ratio"]
                        * model.link_p_nom[chp + " electric"]
                        == network.chps.at[chp, "eta_elec"]
                        / network.chps.at[chp, "c_v"]
                        * model.link_p_nom[chp + " heat"]
                    )

                network.model.chp_nom = Constraint(
                    list(network.chps.index), rule=chp_nom
                )

                def backpressure(model, chp, snapshot):
                    return (
                        network.chps.at[chp, "c_m"]
                        * network.chps.at[chp, "eta_elec"]
                        / network.chps.at[chp, "c_v"]
                        * model.link_p[chp + " heat", snapshot]
                        <= network.chps.at[chp, "eta_elec"]
                        * model.link_p[chp + " electric", snapshot]
                    )
                ...
                network.model.top_iso_fuel_line = Constraint(
                    list(network.chps.index), list(snapshots), rule=top_iso_fuel_line
                )
