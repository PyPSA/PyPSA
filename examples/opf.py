...
for i in range(n*c):
    ...
    #wind generator
    network.add("Generator","Wind %d" % (i),bus=network.buses[str(i)],p_nom=100,source="wind",dispatch="variable",arginal_cost=0)
...
for generator in gas_generators:
    ...
    generator.p_max = pd.Series([generator.p_nom]*len(network.index),network.index)
